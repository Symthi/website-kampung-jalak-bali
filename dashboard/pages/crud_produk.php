...
<div class="crud-list shadow mb-4">
    <div class="list-header">
        <h3 style="margin:0;font-size:1.1rem"><i class="fa fa-list"></i> Daftar Produk (<?php echo $total_produk_all; ?>)</h3>
        <a href="?page=produk&action=add" class="btn btn-success btn-sm">
    ...
    <form method="POST" action="?page=produk" class="search-box">
        ...
        <?php if ($search): ?>
            <a href="?page=produk" class="btn btn-secondary btn-sm" style="padding:0.6rem 1rem;text-decoration:none;color:white">Reset</a>
        <?php endif; ?>
    </form>
    
    <?php if (empty($produk_data)): ?>
        <p class="text-muted" style="padding:1rem">Tidak ada data produk.</p>
    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($produk['gambar']): ?>
                                <img src="<?php echo $base . '/' . $produk['gambar']; ?>" class="thumb-img" onerror="this.src='https://source.unsplash.com/random/50x50/?merchandise'">
                            <?php else: ?>
                        ...
                        <td>
                            <a href="?page=produk&action=edit&id=<?php echo $produk['id_produk']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                            <a href="?page=produk&action=delete&id=<?php echo $produk['id_produk']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin?')"><i class="fa fa-trash"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            ...
            <ul class="pagination justify-content-center">
                <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                    <li class="page-item <?php echo $p == $page ? 'active' : ''; ?>">
                        ...
                        <?php else: ?>
                            <a class="page-link" href="?page=produk&p=<?php echo $p; ?><?php echo $search ? '&cari=' . urlencode($search) : ''; ?>"><?php echo $p; ?></a>
                        <?php endif; ?>
                    </li>
                <?php endfor; ?>
            </ul>
